...
3. Use these commands (room-related commands may be called with universal prefix to prompt for the room):
   ...
   - ~ement-mutual-rooms~ to list rooms in common with another user.
...
+ Write threaded reply to event at point (starts a new thread if it's not already in one): ~s t~
+ Edit message: ~<insert>~
+ Delete message: ~C-k~
...
This release upgrades Ement.el's Matrix protocol support from r0.6.1 toward v1.19.

*Additions*

+ Thread support: threaded replies are recognized via their ~m.thread~ relation and shown inline (with a rich-reply fallback quote, per the spec's recommendation for clients without a dedicated thread view).  A "N replies in thread" summary is shown on thread-root messages, derived from the server's bundled aggregation.  Command ~ement-room-write-thread-reply~ (bound to ~s t~) replies within an existing thread or starts a new one from the event at point.  Unread thread notification counts (~unread_thread_notifications~) are now requested in the sync filter and considered when determining whether a room is unread.
+ Server version negotiation: ~GET /_matrix/client/versions~ is queried at login/sync time, and the negotiated version is used for subsequent API calls (falling back to ~v3~).
+ Access token refresh: refresh tokens are stored and used to automatically renew an expiring access token.
+ Room knocking support (Matrix v1.1+).
+ Server capabilities (~GET /_matrix/client/v3/capabilities~) are fetched after login/reconnect (Matrix v1.1+).
+ Recently-used emoji are tracked via the ~m.recent_emoji~ account data event (Matrix v1.18/MSC4356) and offered as a picker (bound to ~r~ in the reaction picker, e.g. ~s r r~).
+ Command ~ement-mutual-rooms~ (bound to ~u m~), which lists rooms in common with another user via ~GET /_matrix/client/v1/mutual_rooms~ (Matrix v1.19/MSC2666).

*Changes*

+ Client API version prefix upgraded from ~r0~ to ~v3~.
+ Media upload/download upgraded to the authenticated ~/_matrix/client/v1/media/~ endpoints.
+ Mention detection now uses the ~m.mentions~ field (Matrix v1.7) instead of parsing message bodies for MXIDs.
+ Domainless room IDs (Matrix room version 12) are now accepted.
+ ~full_state~ is no longer forced to ~true~ on initial sync, preserving lazy-loading of room members (it was previously overriding ~lazy_load_members~, causing a full membership download on every first login).
+ ~ement-forget-room~ no longer sends a redundant ~/forget~ request when the server advertises the ~m.forget_forced_upon_leave~ capability (Matrix v1.18/MSC4267), since such servers already forget a room automatically once it's left.
+ API errors with ~M_USER_SUSPENDED~, ~M_USER_LOCKED~, or ~M_USER_LIMIT_EXCEEDED~ (Matrix v1.18) now produce a readable, distinguishable message (including any ~info_uri~) instead of a generic status-code error.

*Fixes*

+ Send a valid empty JSON object (~{}~) rather than an empty string as the request body when joining, leaving, or forgetting a room, which some homeservers rejected with ~M_BAD_JSON~.
+ Apply a message's bundled ~m.replace~ (edit) aggregation when the edit event itself isn't separately available, such as when paginating older history.  Previously, such messages were dropped from the timeline entirely rather than showing their edited content.
