...
- **Server capabilities gating**: `ement--fetch-capabilities` (in `ement.el`) fetches `GET /capabilities` after login/reconnect and stores the raw alist on the `ement-server` struct's `capabilities` slot. Features that depend on a capability (e.g. `ement--server-force-forgets-p` checking `m.forget_forced_upon_leave` in `ement-forget-room`) read that slot directly and fall back to the pre-capability behavior if it's nil (fetch failure is non-fatal). Follow this pattern for future capability-gated features rather than probing behavior at the call site.
- **Account data helpers**: `ement-put-account-data` (in `ement-lib.el`) is the shared way to write a session- or room-scoped account data event; reads go through `ement-session-account-data`/`ement-room-account-data`, which is a flat list of `{type, content}` events (not a map), so lookups use `cl-find ... :key (lambda (event) (alist-get 'type event))`. `ement--recent-emoji`/`ement--update-recent-emoji` (`m.recent_emoji`, MSC4356) are the most recent example of this pattern; mirror them for new account-data-backed features instead of inventing new storage.
